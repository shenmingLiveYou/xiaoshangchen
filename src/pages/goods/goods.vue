<template>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          <!-- 属性详情 -->
          <view class="item" v-for="item in goodsData?.details.properties" :key="item">
  ...
  <uni-popup ref="popup" type="bottom" background-color="#fff">
    <AddressPanel
      ...
      @close="popup?.close()"
      @address="address"
      :memberAddress="(memberAddress as AddressItem[])"
    />
...
// 获取地址信息数据
const memberAddress = ref<AddressItem[]>()

const getMemberAddress = async () => {
  const res = await getMemberAddressAPI()

  memberAddress.value = res.result
}
...
const addressText = computed(() => {
  return addressItem.value
    ? addressItem.value?.fullLocation + addressItem.value?.address
    : '请选择配送地址'
})
